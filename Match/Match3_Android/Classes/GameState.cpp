...
bool GameState::init()
{
    if (CCNode::init())
    {
        ...
        CCLog("shauket:: totalLive %d",totalLife);
...
void GameState::updateScoreToServer()
{
    if (isNetWorkAvailAble_ == false) {
        CCLog("shauket:: isnetworkAvailAble %d",isNetWorkAvailAble_);
        return;
    }
    int totalScore = 0;
   ...
   CCLog("shauket:: in update ScoreToServer");
...
void GameState::giveLife(int quantity, bool add)
{
	...
	CCLog("shauket:: quantity %d",quantity);
    if (add){
        totalLife+=quantity;
    	CCLog("shauket:: totallife1 %d",totalLife);

    }
    else
    {
        totalLife=totalLife-quantity;
    	CCLog("shauket:: totallife2 %d",totalLife);
    }
    if(totalLife>MAX_LIVES_COUNT){
        totalLife = MAX_LIVES_COUNT;
    }
	CCLog("shauket:: totallife3 %d",totalLife);
