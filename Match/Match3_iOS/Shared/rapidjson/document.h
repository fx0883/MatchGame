...
public:
	...
	GenericValue(Type type) {
		static const unsigned defaultFlags[7] = { kNullFlag, kFalseFlag,
				kTrueFlag, kObjectFlag, kArrayFlag, kConstStringFlag,
	...
	~GenericValue() {
		if (Allocator::kNeedFree) { // Shortcut by Allocator's trait
			...
			case kObjectFlag:
				for (Member* m = data_.o.members; m != data_.o.members
						+ data_.o.size; ++m) {
					m->name.~GenericValue();
	...
	GenericValue& operator=(Type value) const {
		this->~GenericValue();
		new (this) GenericValue(value);
		return *this;
	}
...
//	//@{
//
//	GenericValue& GetGenericValueObj() {
//		//GenericValue temp = new (value) GenericValue(value);
//		GenericValue<Encoding, Allocator> temp;
//		new (temp) GenericValue();
//		return *temp;
////		return *temp;
//	}
//
//	GenericValue& GetGenericValueObj()const {
//		(*this).GetGenericValueObj();
//		}
...
//	void ReplaceObjectKey(const Ch* previousKey, const Ch* newKey) {
//		if (Member* member = FindMember(previousKey)) {
//			member->name = newKey;
//		}
//	}

	void ReplaceObjectKey(const char* previousKey, const char* newKey) const {
//		(*this).ReplaceObjectKey(previousKey, newKey);
//		if (const Member* member = FindMember(previousKey)) {
//			member->name.SetString(newKey);
//		}
		const Object& o = data_.o;
		for (Member* member = o.members; member != data_.o.members+ data_.o.size; ++member)
		{
			CCString * ss = new CCString(member->name.GetString());
			if(strcmp(ss->getCString(),previousKey) == 0)
			{
				member->name.SetString(newKey);
			}
			ss->release();
	...
	GenericValue& AddMember(GenericValue& name, GenericValue& value,
		...
		if (o.size >= o.capacity) {
			if (o.capacity == 0) {
				...
				o.members = (Member*) allocator.Malloc(
...
//		RAPIDJSON_ASSERT(flags_ & kIntFlag);
		if(IsString())
		{
			const char* stringVal = GetString();
...
//		RAPIDJSON_ASSERT(IsNumber());
	...
	const Ch* GetString() const {
		...
		{
			return CCString::createWithFormat("%i",GetInt())->getCString();
		}
		else if (IsInt64())
		{
			return CCString::createWithFormat("%lld",GetInt64())->getCString();
		}
		else if (IsDouble())
		{
			return CCString::createWithFormat("%f",GetDouble())->getCString();
		}
		else if (IsBool())
		{
			if(GetBool() == true)
			{
				return CCString::create("true")->getCString();
			}
			else
			{
				return CCString::create("false")->getCString();
			}
		}
		return "";
	}

	CCArray* allKeys() const {
		CCArray * allKeys = new CCArray();
		allKeys->autorelease();
		const Object& o = data_.o;
		for (Member* member = o.members; member != data_.o.members+ data_.o.size; ++member)
		{
	...
	bool hasKey(const char *key) const
	{
		...
		for (Member* member = o.members; member != data_.o.members+ data_.o.size; ++member)
		{
            if(!member)//Wasay added check if name is null
                return false;
			CCString * ss = new CCString(member->name.GetString());
			if(strcmp(ss->getCString(),key) == 0)
				return true;
			ss->release();
		}
		return false;
...
private:
	...
	union Data {
		...
		Object o;
		Array a;
	}; // 12 bytes in 32-bit mode cv吃, 16 bytes in 64-bit mode
	...
	Member* FindMember(const Ch* name) {
		...
		Object& o = data_.o;
		for (Member* member = o.members; member != data_.o.members
				+ data_.o.size; ++member)
			if (name[member->name.data_.s.length] == '\0' && memcmp(
					member->name.data_.s.str, name,
					member->name.data_.s.length * sizeof(Ch)) == 0)
				return member;
